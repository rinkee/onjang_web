(() => {
  ...
  (function () {
    ...
    let l = p(u());
    function u() {
      let n = document.querySelector('base');
    ...
    async function d(n, e, r) {
      ...
      let t,
        i = new Promise((o, s) => {
          t = setTimeout(() => {
            s(new Error(`${r} took more than ${e}ms to resolve. Moving on.`, { cause: d }));
          }, e);
        });
    ...
    class y {
      constructor(e, r = 'flutter-js') {
        let t = e || [/\.js$/];
        window.trustedTypes &&
          (this.policy = trustedTypes.createPolicy(r, {
            createScriptURL: function (i) {
              let o = new URL(i, window.location),
                s = o.pathname.split('/').pop();
...
Read more: https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts`)),
        ...
        let { serviceWorkerVersion: r, serviceWorkerUrl: t = `${l}flutter_service_worker.js?v=${r}`, timeoutMillis: i = 4e3 } = e,
          o = t;
        this._ttPolicy != null && (o = this._ttPolicy.createScriptURL(o));
        let s = navigator.serviceWorker
          .register(o)
          .then((c) => this._getNewServiceWorker(c, r))
          .then(this._waitForServiceWorkerActivation);
        return d(s, i, 'prepareServiceWorker');
      }
      async _getNewServiceWorker(e, r) {
        if (!e.active && (e.installing || e.waiting))
          return console.debug('Installing/Activating first service worker.'), e.installing || e.waiting;
        if (e.active.scriptURL.endsWith(r)) return console.debug('Loading from existing service worker.'), e.active;
        {
          let t = await e.update();
          return console.debug('Updating service worker.'), t.installing || t.waiting || t.active;
        }
      }
      async _waitForServiceWorkerActivation(e) {
        ...
        return new Promise((r, t) => {
          e.addEventListener('statechange', () => {
            e.state == 'activated' && (console.debug('Activated new service worker.'), r());
          });
        });
      }
    }
    class f {
      ...
      async loadEntrypoint(e) {
        let { entrypointUrl: r = `${l}main.dart.js?version=2 `, onEntrypointLoaded: t, nonce: i } = e || {};
      ...
      didCreateEngineInitializer(e) {
        typeof this._didCreateEngineInitializerResolve == 'function' &&
          ...
          delete a.loader.didCreateEngineInitializer),
          typeof this._onEntrypointLoaded == 'function' && this._onEntrypointLoaded(e);
      }
      _loadEntrypoint(e, r, t) {
        ...
        if (!this._scriptLoaded) {
          ...
          if (i) console.debug('Injecting <script> tag. Using callback.'), (this._onEntrypointLoaded = r), document.body.append(o);
          else
    ...
    class v {
      async loadEntrypoint(e) {
        let { serviceWorker: r, ...t } = e || {},
          i = new y(),
          o = new g();
        o.setTrustedTypesPolicy(i.policy),
          await o.loadServiceWorker(r).catch((c) => {
            console.warn('Exception while loading service worker:', c);
          });
        let s = new f();
        return (
          s.setTrustedTypesPolicy(i.policy), (this.didCreateEngineInitializer = s.didCreateEngineInitializer.bind(s)), s.loadEntrypoint(t)
        );
      }
    }
    a.loader = new v();
